	...
	<div class="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="max-w-md w-full bg-white p-6 sm:p-10 rounded-2xl sm:rounded-3xl shadow-xl border border-slate-100">
			...
			<div class="text-center mb-6 sm:mb-8">
				<div>
					<img src="../assets/logo_pnj.png" alt="Logo PNJ" class="mx-auto h-16 sm:h-20 w-auto mb-3 sm:mb-4 drop-shadow-md" />
				</div>
			...
			<form class="space-y-5 sm:space-y-6" @submit.prevent="handleSubmit">
				<div class="space-y-4 sm:space-y-5">
					<div>
						<label class="block text-xs sm:text-sm font-bold text-slate-700 mb-1 ml-1">
							Email PNJ<span class="text-red-500"> *</span>
						</label>
						<input v-model="form.email" type="email" required
							class="appearance-none relative block w-full px-4 py-3 sm:py-3.5 border border-slate-200 placeholder-slate-400 text-slate-900 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm"
					...
					<div>
						...
						<div class="relative">
							...
							<button type="button" @click="togglePassword"
								class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-blue-600 transition-colors cursor-pointer active:scale-95">
			...
			<div class="text-center mt-6 sm:mt-8 space-y-4">
				<p class="text-xs sm:text-sm font-medium text-slate-500">
					Belum punya akun?
					<router-link to="/register" @click="goToRegister" class="text-blue-600 font-bold hover:text-blue-700 transition-colors">
						Daftar di sini
					</router-link>
				</p>
