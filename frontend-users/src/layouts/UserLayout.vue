<template>
  <div class="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <div class="w-full px-6 md:px-10 lg:px-12">
        <div class="flex justify-between items-center h-16">
          ...
          <div class="flex items-center gap-3">
            <div>
              <img src="../assets/logo_pnj.png" alt="Logo PNJ" class="mx-auto h-10 w-auto drop-shadow-md" />
            </div>
            <span class="font-black text-gray-900 text-lg tracking-tight hidden sm:block">SI-LAB TIK</span>
          </div>
          ...
          <div class="flex items-center gap-3 md:gap-4">
            ...
            <button @click="handleLogout"
              class="hidden md:flex p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors cursor-pointer"
              title="Keluar">
              <ArrowRightOnRectangleIcon class="w-5 h-5" />
            </button>

            <button @click="isMobileMenuOpen = !isMobileMenuOpen"
              class="md:hidden p-2 text-gray-600 rounded-xl hover:bg-gray-100 transition-colors cursor-pointer ml-1">
      ...
      <transition name="fade">
        <div v-if="isMobileMenuOpen" class="md:hidden bg-white border-t border-gray-100 shadow-lg absolute w-full">
          <div class="px-4 pt-2 pb-4 space-y-1">
            <router-link @click="isMobileMenuOpen = false" to="/dashboard"
              class="block px-4 py-3 rounded-xl text-base font-bold text-gray-700 hover:text-blue-600 hover:bg-blue-50 transition-colors"
              active-class="bg-blue-50 text-blue-600">Beranda</router-link>
            <router-link @click="isMobileMenuOpen = false" to="/catalog"
              class="block px-4 py-3 rounded-xl text-base font-bold text-gray-700 hover:text-blue-600 hover:bg-blue-50 transition-colors"
              active-class="bg-blue-50 text-blue-600">Katalog Alat</router-link>
            <router-link @click="isMobileMenuOpen = false" to="/riwayat"
              class="block px-4 py-3 rounded-xl text-base font-bold text-gray-700 hover:text-blue-600 hover:bg-blue-50 transition-colors"
              active-class="bg-blue-50 text-blue-600">Peminjaman Saya</router-link>

            <div class="my-2 border-t border-gray-100"></div>

            <router-link @click="isMobileMenuOpen = false" to="/profile"
              class="block px-4 py-3 rounded-xl text-base font-bold text-gray-700 hover:text-blue-600 hover:bg-blue-50 transition-colors"
              active-class="bg-blue-50 text-blue-600">Profil Saya</router-link>
            <button @click="handleLogout"
              class="w-full text-left block px-4 py-3 rounded-xl text-base font-bold text-red-600 hover:bg-red-50 transition-colors cursor-pointer">Keluar
              Aplikasi</button>
          </div>
        </div>
      </transition>
    ...
    <footer class="bg-white border-t border-gray-200 mt-auto">
      <div
        class="w-full px-6 md:px-10 lg:px-12 py-6 flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
