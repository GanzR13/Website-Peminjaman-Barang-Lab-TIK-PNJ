    ...
    <Teleport to="body">
        <transition name="fade">
            <div v-if="isOpen" class="fixed inset-0 z-999 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-4" @click.self="closeModal">
                ...
                <div class="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-4xl p-0 overflow-hidden relative flex flex-col max-h-[92vh] sm:max-h-[85vh] lg:max-h-[75vh]">
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-12 w-full flex-1 overflow-hidden">
                        ...
                        <!-- Gambar -->
                        <div class="lg:col-span-5 bg-slate-50 lg:border-r border-slate-100 flex flex-col items-center justify-start lg:overflow-y-auto custom-scrollbar">
                            
                            <!-- Mobile: gambar horizontal kecil -->
                            <div class="w-full lg:hidden px-5 pt-3 pb-4">
                                <div class="w-full h-52 rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                                    <img :src="getImageUrl(data?.gambar)" :alt="data?.nama_barang"
                                        loading="lazy" class="w-full h-full object-cover" />
                                </div>
                            </div>

                            <!-- Desktop: gambar square -->
                            <div class="hidden lg:block w-full p-6 md:p-8">
                                <div class="w-full aspect-square rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden group lg:sticky lg:top-4">
                                    <img :src="getImageUrl(data?.gambar)" :alt="data?.nama_barang"
                        ...
                        <div class="lg:col-span-7 flex flex-col bg-white overflow-y-auto custom-scrollbar flex-1">
                            ...
                            <div class="p-5 md:p-8 flex-1">
                                ...
                                <!-- Nama Barang -->
                                <h1 class="text-xl md:text-3xl font-black text-slate-900 leading-tight pr-8 mb-5">
                                    {{ data?.nama_barang }}
                                </h1>

                                <!-- Stok -->
                                <div class="bg-blue-50/50 p-4 md:p-5 rounded-2xl border border-blue-100/50 w-fit pr-10 mb-6">
                                    <p class="text-[11px] font-bold text-blue-500/80 uppercase tracking-widest mb-1">Stok Gudang</p>
                                ...
                                <!-- Deskripsi -->
                                <div>
                                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-3 mb-4">
