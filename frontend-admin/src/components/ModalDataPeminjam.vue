<template>
    <transition name="fade">
        <div v-if="isOpen && mode === 'detail'" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-0 overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    ...
                    <button @click="$emit('close')" class="w-full mt-4 px-6 py-3 text-sm font-black text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-2xl transition cursor-pointer">
                        Tutup Panel Detail
                    </button>
                </div>
            </div>
    ...
    <transition name="fade">
        <div v-if="isOpen && mode === 'form'" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" @click.self="closeAllDropdowns">
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto custom-select relative">
                ...
                <form @submit.prevent="handleSave" class="space-y-4">
                    ...
                    <div v-if="formData.kategori === 'Mahasiswa'" class="grid grid-cols-2 gap-4">
                        ...
                        <div class="relative custom-select">
                            ...
                            <button type="button" @click.stop="toggleDropdown('prodi')" class="w-full flex items-center justify-between px-4 py-2.5 border border-gray-200 rounded-xl bg-white text-sm font-bold cursor-pointer">
                                <span>{{ refProdi.find((p) => p.id === formData.prodi_id)?.nama_prodi || "Pilih Prodi" }}</span>
                    ...
                    <div class="flex justify-end gap-3 pt-6 border-t mt-4">
                        <button type="button" @click="$emit('close')" class="px-6 py-2.5 text-sm font-bold text-gray-500 hover:bg-gray-100 rounded-2xl transition cursor-pointer">
                            Batalkan
                        </button>
...
const props = defineProps({
    ...
    mode: { type: String, default: 'form' }, // 'form' atau 'detail'
