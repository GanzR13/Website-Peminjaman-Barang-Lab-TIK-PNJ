<template>
    <transition name="fade">
        <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-3 sm:p-4" @click.self="$emit('close')">
            
            <div class="bg-white rounded-2xl md:rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col relative overflow-hidden">
                ...
                <div v-if="item" class="flex flex-col md:grid md:grid-cols-2 flex-1 overflow-y-auto md:overflow-hidden tracking-tight relative">

                    <div class="bg-slate-50 p-5 md:p-8 border-b md:border-b-0 md:border-r border-slate-200 md:overflow-y-auto shrink-0">
                        <h3 class="text-lg font-black text-slate-900 mb-4 md:mb-6 flex items-center gap-2 pr-10 md:pr-0">
                        ...
                        <a :href="getImageUrl(item.foto_bukti)" target="_blank"
                            ...
                            <img :src="getImageUrl(item.foto_bukti)" class="w-full h-full object-cover md:object-contain" />
                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center font-bold text-white text-xs uppercase tracking-widest gap-2">
                        ...
                        <div class="space-y-4 text-sm font-medium">
                            <div class="bg-white p-3 rounded-xl border border-slate-100">
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Barang Bermasalah</p>
                                <p class="font-bold text-slate-800">{{ item.barang?.nama_barang }} ({{ item.jumlah }} Unit)</p>
                            </div>
                            <div>
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">Kronologi Kejadian</p>
                                <div class="p-3 md:p-4 bg-white border border-slate-200 rounded-xl text-slate-600 italic leading-relaxed text-xs md:text-sm">
                                    "{{ item.deskripsi }}"
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-white p-5 md:p-8 flex flex-col md:overflow-y-auto">
                        
                        <div class="mb-5 md:mb-6 pb-4 border-b border-slate-100">
                            <span :class="item.jenis_laporan === 'Kerusakan' ? 'bg-red-100 text-red-700 ring-red-200' : 'bg-orange-100 text-orange-700 ring-orange-200'"
                        ...
                        <div v-if="item.status === 'Selesai Diperbaiki'" class="mb-5 p-3 md:p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex gap-3 items-start animate-fade-in">
                        ...
                        <div v-if="item.status === 'Rusak Total'" class="mb-5 p-3 md:p-4 bg-slate-900 text-white border border-slate-800 rounded-xl flex gap-3 items-start animate-fade-in">
                        ...
                        <form @submit.prevent="handleSave" class="flex-1 flex flex-col gap-5">
                            <div>
                                <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Update Status & Tindakan <span class="text-red-500">*</span></label>

                                <select v-model="localForm.status" :disabled="isStatusLockedPermanently" required
                                    class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-4 focus:ring-orange-50 focus:border-orange-500 outline-none text-sm font-bold text-slate-700 bg-slate-50 disabled:opacity-75 disabled:cursor-not-allowed transition-all">
                            ...
                            <div class="flex-1 flex flex-col">
                                <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Catatan Perbaikan / Servis <span class="text-red-500">*</span></label>
