  ...
  <div class="p-4 md:p-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-1 font-medium">
          Kelola data kelas yang digunakan pada data mahasiswa.
        </p>
      </div>
    ...
    <div class="bg-white border border-slate-100 rounded-2xl md:rounded-3xl shadow-sm overflow-hidden">
      ...
      <div class="p-4 md:p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-slate-500 font-medium mt-1">
            Total {{ filteredKelas.length }} data kelas.
          </p>
        </div>
      ...
      <div v-if="isLoading" class="p-10 flex flex-col items-center justify-center text-center">
        ...
        <p class="text-sm font-bold text-slate-500">Memuat data kelas...</p>
      </div>

      <!-- Empty -->
      <div v-else-if="filteredKelas.length === 0" class="p-10 text-center">
        ...
        <p class="text-sm text-slate-500 font-medium mt-1">
          Belum ada data kelas atau kata kunci pencarian tidak sesuai.
        </p>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50 text-slate-500 text-[11px] uppercase tracking-widest border-b border-slate-100">
              ...
              <th class="px-6 py-4 font-black">Nama Kelas</th>
...
const API_URL = 'http://localhost:3000/api/ref/kelas';
...
const fetchKelas = async () => {
  ...
  try {
    ...
    kelasList.value = Array.isArray(response.data)
      ? response.data
      : response.data?.data || [];
  } catch (error) {
    console.error('Gagal mengambil data kelas:', error);
    showAlert(error.response?.data?.message || 'Gagal mengambil data kelas.', 'error');
  } finally {
...
const submitForm = async () => {
  if (!form.value.nama_kelas.trim()) {
    showAlert('Nama kelas wajib diisi.', 'warning');
    return;
  }
  ...
  try {
    const payload = {
      nama_kelas: form.value.nama_kelas.trim(),
    };
    ...
    if (isEditMode.value) {
      ...
      showAlert('Data kelas berhasil diperbarui.', 'success');
    } else {
      await axios.post(API_URL, payload, {
        headers: authHeaders(),
      });

      showAlert('Data kelas berhasil ditambahkan.', 'success');
    }
    ...
    showAlert(error.response?.data?.message || 'Gagal menyimpan data kelas.', 'error');
  } finally {
  ...
  showConfirm(
    ...
    async () => {
      try {
        ...
        showAlert('Data kelas berhasil dihapus.', 'success');
        await fetchKelas();
      } catch (error) {
        console.error('Gagal menghapus data kelas:', error);
        showAlert(error.response?.data?.message || 'Gagal menghapus data kelas.', 'error');
      }
    },
    null,
    'Ya, Hapus',
