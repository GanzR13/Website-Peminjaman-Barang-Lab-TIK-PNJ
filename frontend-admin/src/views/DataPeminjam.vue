<template>
    <div class="p-4 md:p-8 h-full flex flex-col relative animate-fade-in bg-slate-50 tracking-tight">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 md:mb-8">
            <div>
                <h2 class="text-xl md:text-2xl font-black text-gray-900 tracking-tight">Manajemen Peminjam</h2>
                <p class="text-gray-600 mt-1 text-xs md:text-sm">Kelola data, profil, dan kategori peminjam laboratorium (Mahasiswa
                    & Dosen).</p>
            </div>

            <button @click="fetchUsers"
                class="w-full sm:w-auto flex justify-center items-center gap-2 px-4 py-2.5 text-gray-600 bg-white border border-gray-200 hover:text-emerald-600 hover:border-emerald-300 hover:bg-emerald-50 rounded-xl transition-all shadow-sm shrink-0 font-semibold text-sm cursor-pointer active:scale-95"
                title="Refresh Data">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                    <div class="relative w-full sm:w-64">
                        ...
                        <input v-model="searchQuery" type="text" placeholder="Cari nama, NIM/NIP..."
                            class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 outline-none transition-all bg-gray-50 focus:bg-white" />
                    </div>
                ...
                <div v-if="filteredUsers.length > 0" class="md:hidden flex flex-col gap-3 p-4 bg-gray-50/30">
                    <div v-for="user in filteredUsers" :key="user.id" class="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm relative">
                        <div class="flex justify-between items-start mb-3">
                            <span class="px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wide ring-1"
                            ...
                            <span v-if="checkVerified(user.user?.email_verified)" class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-600 border border-emerald-200 text-[9px] font-bold uppercase tracking-wider">
                                Terverifikasi
                            </span>
                            <span v-else class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-red-50 text-red-600 border border-red-200 text-[9px] font-bold uppercase tracking-wider">
                                Belum Verif
                            </span>
                        </div>

                        <div class="flex items-center gap-4 mb-4">
                            <div class="w-12 h-12 rounded-full bg-linear-to-tr from-emerald-500 to-teal-400 flex items-center justify-center text-white font-bold text-lg shadow-md uppercase shrink-0">
                                {{ user.nama_lengkap?.charAt(0) || "?" }}
                            </div>
                            <div class="flex-1 overflow-hidden">
                                <h3 class="font-bold text-gray-900 leading-tight truncate">{{ user.nama_lengkap }}</h3>
                                <p class="text-[10px] text-gray-500 font-mono mt-0.5 truncate">{{ user.nim ? "NIM: " + user.nim : "NIP: " + user.nip }}</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-xl p-3 mb-4 space-y-2 border border-gray-100">
                            <div class="flex flex-col gap-1 text-xs">
                                ...
                                <span class="font-medium text-gray-700 break-all">{{ user.user?.email || "-" }}</span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <button @click="handleDelete(user.id)" class="py-2 bg-red-50 text-red-600 font-bold rounded-lg text-xs hover:bg-red-100 transition-colors active:scale-95">Hapus</button>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50 text-gray-500 text-[11px] uppercase tracking-widest border-y border-gray-100">
                                <th class="px-6 py-4 font-black">Profil Peminjam</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-emerald-50/30 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 rounded-full bg-linear-to-tr from-emerald-500 to-teal-400 flex items-center justify-center text-white font-bold text-sm shadow-md shadow-emerald-200 uppercase shrink-0">
                                            {{ user.nama_lengkap?.charAt(0) || "?" }}
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col items-start gap-1.5">
                                        <span class="text-sm text-gray-600 font-medium">{{ user.user?.email || "-" }}</span>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center items-center gap-2">
                                        <button @click="openDetailModal(user)" class="text-indigo-600 hover:text-indigo-800 font-bold text-xs bg-indigo-50 hover:bg-indigo-100 px-3 py-2 rounded-lg transition-colors cursor-pointer active:scale-95">
                                            Detail
                                        </button>
                                        <button @click="openEditModal(user)" class="text-blue-600 hover:text-blue-800 font-bold text-xs bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded-lg transition-colors cursor-pointer active:scale-95">
                                            Edit
                                        </button>
                                        <button @click="handleDelete(user.id)" class="text-red-500 hover:text-red-700 font-bold text-xs bg-red-50 hover:bg-red-100 px-3 py-2 rounded-lg transition-colors cursor-pointer active:scale-95">
                                            Hapus
                                        </button>
                                    </div>
                                </td>
                ...
                <div v-if="totalItems > 0 && !searchQuery" class="bg-gray-50 px-4 md:px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between mt-auto gap-4">
                    ...
                    <div class="flex items-center justify-between w-full sm:w-auto gap-4 text-sm text-gray-600">
                        ...
                        <div class="relative custom-select w-full sm:w-auto">
                            <button @click.stop="toggleDropdown('limit')" class="flex items-center justify-between w-full sm:w-28 px-4 sm:px-3 py-2.5 sm:py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-700 hover:border-emerald-400 transition-all outline-none cursor-pointer shadow-sm">
                            ...
                            <transition name="fade">
                                <div v-if="activeDropdown === 'limit'" class="absolute bottom-full left-0 right-0 mb-2 bg-white border border-gray-100 rounded-xl shadow-xl overflow-hidden py-1 z-30">
                                    <button v-for="opt in [5, 10, 25, 50]" :key="opt" @click="pilihLimit(opt)" class="w-full px-4 py-2 text-sm text-left hover:bg-emerald-50 cursor-pointer" :class="limit === opt ? 'text-emerald-600 font-black bg-emerald-50/50' : 'text-gray-600 font-bold'">
                                        {{ opt }} Data
                                    </button>
                                </div>
                            </transition>
                    ...
                    <div class="flex items-center justify-between w-full sm:w-auto gap-4">
                        <span class="text-xs font-bold text-gray-500 uppercase tracking-widest text-center flex-1 sm:flex-none">
                            Hal <span class="text-emerald-600 font-black">{{ currentPage }}</span> / {{ totalPages }}
                        </span>
                        
                        <div class="flex gap-2 shrink-0">
                            <button @click="prevPage" :disabled="currentPage === 1" class="p-2 sm:px-3 sm:py-2 border border-gray-200 rounded-xl bg-white text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm cursor-pointer flex items-center gap-1">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                                <span class="hidden sm:inline text-xs font-bold">Prev</span>
                            </button>

                            <button @click="nextPage" :disabled="currentPage === totalPages" class="p-2 sm:px-3 sm:py-2 border border-gray-200 rounded-xl bg-white text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm cursor-pointer flex items-center gap-1">
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                showAlert(
                    error.response?.data?.message || 'Gagal menghapus data peminjam.',
                    'error'
                );
            }
        },
