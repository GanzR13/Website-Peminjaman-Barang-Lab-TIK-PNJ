<template>
    <!-- Template kamu SUDAH BAGUS SEKALI, tidak perlu saya ubah agar desain Tailwind-mu tetap utuh -->
                            ...
                            <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-emerald-50/30 transition">
                                ...
                                <td class="px-8 py-5">
                                    <span
                                        ...
                                        :class="user.user?.Role?.nama_role === 'Dosen'
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <div class="flex gap-2">
                            <button @click="prevPage" :disabled="currentPage === 1"
                                class="p-2 border border-gray-200 rounded-xl bg-white disabled:opacity-30 cursor-pointer hover:text-emerald-600 hover:border-emerald-300 transition-colors shadow-sm">
                ...
                <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-0 overflow-hidden">
                    ...
                    <div class="p-6 space-y-3">
                        <div class="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Identitas ({{
                                detailUser.nim ? "NIM" : "NIP" }})</p>
                            <p class="text-gray-900 font-bold font-mono">{{ detailUser.nim || detailUser.nip || "-" }}
                            </p>
                        </div>
                        <div v-if="detailUser.nim" class="grid grid-cols-2 gap-3">
                            <div class="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                                ...
                                <p class="text-gray-900 font-bold text-xs">{{ detailUser.angkatan }} - {{
                                    detailUser.nama_prodi || detailUser.Prodi?.nama_prodi || "-" }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                                <p class="text-[10px] font-black text-gray-400 uppercase mb-1">Kelas</p>
                                <p class="text-gray-900 font-bold text-xs">{{ detailUser.nama_kelas ||
                ...
                <form @submit.prevent="saveData" class="space-y-4">
                    ...
                    <div v-if="formData.kategori === 'Mahasiswa'" class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">NIM (Nomor
                        ...
                        <div class="relative custom-select">
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Program
                                Studi</label>
                            <button type="button" @click.stop="toggleDropdown('prodi')"
                                ...
                                <span>{{ref_prodi.find((p) => p.id === formData.prodi_id)?.nama_prodi || "Pilih Prodi"
                                    }}</span>
                            ...
                            <transition name="fade">
                                <div v-if="activeDropdown === 'prodi'"
                                    ...
                                    <button v-for="p in ref_prodi" :key="p.id" type="button"
                        ...
                        <div class="relative custom-select">
                            <label
                                class="block text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Angkatan</label>
                            ...
                            <transition name="fade">
                                <div v-if="activeDropdown === 'angkatan'"
                                    ...
                                    <button v-for="y in years" :key="y" type="button" @click="pilihAngkatan(y)"
                                        class="w-full px-4 py-2 text-sm text-left hover:bg-emerald-50 font-medium">
                                        {{ y }}
                                    </button>
                                </div>
                        ...
                        <div class="col-span-2 relative custom-select">
                            ...
                            <button type="button" @click.stop="toggleDropdown('kelas')"
                                class="w-full flex items-center justify-between px-4 py-2.5 border border-gray-200 rounded-xl bg-white text-sm font-bold cursor-pointer">
                                <span>{{ref_kelas.find((k) => k.id === formData.kelas_id)?.nama_kelas || "Pilih Kelas"
                                    }}</span>
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <transition name="fade">
                                <div v-if="activeDropdown === 'kelas'"
                                    class="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-100 rounded-xl shadow-xl z-50 max-h-40 overflow-y-auto">
                                    <button v-for="k in ref_kelas" :key="k.id" type="button"
                    ...
                    <div v-if="formData.kategori === 'Dosen'">
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Nomor Induk
                            Pegawai (NIP)</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Email
                                Institusi</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Nomor
                                Telepon</label>
                    ...
                    <div v-if="!isEditMode">
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Password
                            Awal</label>
...
<script setup>
import { ref, onMounted, onUnmounted, computed, reactive } from "vue"; // Ditambah onUnmounted
...
// FIXED 1: Mencegah Memory Leak saat komponen hancur/pindah halaman
onUnmounted(() => {
...
// FIXED 2 & 3: Optimasi Payload dan Success Alert
const saveData = async () => {
    try {
        // Clone formData agar state reaktif tidak terganggu
        const payload = { ...formData, role_id: formData.kategori === "Mahasiswa" ? 5 : 4 };

        // Bersihkan data sampah tergantung kategorinya agar Backend tidak komplain
...
// FIXED 4: Implementasi useAlert untuk konfirmasi Hapus (Bukan confirm bawaan browser)
