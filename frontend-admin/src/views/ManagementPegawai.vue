	...
	<div class="flex min-h-screen bg-gray-50 relative">
		...
		<div class="flex-1 flex flex-col h-screen overflow-hidden">
									...
									<tr
										...
										<td class="px-8 py-5">
											<div class="flex items-center">
												<div
													class="w-10 h-10 rounded-full bg-linear-to-tr from-blue-600 to-indigo-500 flex items-center justify-center text-white font-bold mr-4 text-sm shadow-md shadow-blue-200 uppercase">
													{{ user.nama_lengkap?.charAt(0) || "?" }}
												</div>
												<div>
													<span
														class="font-bold text-gray-900 block leading-tight mb-1"
														>{{ user.nama_lengkap || "Belum Ada Profil" }}</span
													>
													<span
										...
										<td class="px-8 py-5">
											<div class="flex flex-col items-start gap-1.5">
												...
												<span
													...
													">
													{{
														user.user?.Role?.level_akses?.replace("_", " ") ||
														"N/A"
													}}
												</span>
											</div>
										...
										<td class="px-8 py-5 text-center">
											<div class="flex justify-center items-center gap-2">
												<button
													...
													class="text-indigo-600 hover:text-indigo-800 font-semibold text-sm hover:underline bg-indigo-50 hover:bg-indigo-100 px-3 py-1 rounded-lg transition-colors">
													Detail
												</button>
												<button
													@click="openEditModal(user)"
													class="text-blue-600 hover:text-blue-800 font-semibold text-sm hover:underline bg-blue-50 hover:bg-blue-100 px-3 py-1 rounded-lg transition-colors">
													Edit
												</button>
												<button
													@click="deleteUser(user.id)"
													class="text-red-500 hover:text-red-700 font-semibold text-sm hover:underline bg-red-50 hover:bg-red-100 px-3 py-1 rounded-lg transition-colors">
													Hapus
												</button>
											</div>
										</td>
						...
						<div
							...
							<div class="flex items-center gap-6">
								<span
									...
									Halaman <span class="text-blue-600">{{ currentPage }}</span> /
									{{ totalPages }}
								</span>
								...
								<div class="flex gap-2">
									<button
										...
										class="p-2 border border-gray-200 rounded-xl bg-white text-gray-600 hover:text-blue-600 hover:border-blue-300 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm">
										<svg
											xmlns="http://www.w3.org/2000/svg"
											class="h-5 w-5"
											fill="none"
			...
			<div
				...
				<div
					class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-0 overflow-hidden transform transition-all">
					<div
						class="bg-linear-to-r from-blue-600 to-indigo-600 p-6 flex items-center gap-4">
						<div
							class="w-16 h-16 rounded-full bg-white/20 flex items-center justify-center text-white font-bold text-2xl shadow-inner border border-white/30 uppercase">
						...
						<div class="text-white">
							...
							<p
								class="text-blue-100 text-sm opacity-90 uppercase tracking-wider mt-1 text-[10px] font-bold">
								{{
									detailUser.user?.Role?.level_akses?.replace("_", " ") ||
									"PENGELOLA"
								}}
							</p>
						</div>
					...
					<div class="p-6 space-y-3">
						...
						<div
							class="bg-gray-50 rounded-xl p-4 border border-gray-100 flex justify-between items-center">
							<div>
								<p
									class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">
									No. Telepon / WA
							...
							<div class="text-right">
								...
								<span
									v-if="detailUser.user?.email_verified"
									class="px-2 py-1 rounded text-[10px] font-black uppercase tracking-wide bg-emerald-100 text-emerald-700"
									>Terverifikasi</span
								>
								<span
									v-else
									class="px-2 py-1 rounded text-[10px] font-black uppercase tracking-wide bg-orange-100 text-orange-700"
									>Unverified</span
								>
							</div>
						</div>
						...
						<div
							...
							<div>
								...
								<p class="text-gray-900 font-bold capitalize">
									{{ detailUser.user?.Role?.nama_role || "Pegawai" }}
								</p>
							</div>
							<div class="text-right">
								<p
								...
								<span
									class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wide"
									:class="
										detailUser.user?.Role?.level_akses === 'super_admin'
						...
						<div class="flex justify-end pt-2">
							<button
								...
								class="px-6 py-2.5 text-sm font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl transition">
								Tutup Detail
							</button>
						</div>
					</div>
			...
			<div
				...
				class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
				<div
					class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 overflow-hidden transform transition-all">
					...
					<form @submit.prevent="saveData" class="space-y-4">
						...
						<div v-if="!isEditMode">
							...
							<div class="relative">
								<input
									...
									required
									class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none pr-10" />
								<button
									type="button"
						...
						<div
							...
							<button
								type="button"
								@click="closeModal"
								class="px-5 py-2 text-sm font-bold text-gray-600 hover:bg-gray-100 rounded-xl transition">
								Batal
							</button>
							<button
								type="submit"
								class="px-5 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-lg shadow-blue-200 transition">
								Simpan Data
							</button>
						</div>
					</form>
...
const saveData = async () => {
	try {
		if (isEditMode.value) {
			alert("Fungsi Update Frontend berjalan! (Menunggu Backend)");
		} else {
			alert("Fungsi Create Frontend berjalan! (Menunggu Backend)");
		}
		closeModal();
	} catch (err) {
...
const deleteUser = async (id) => {
	if (confirm("Apakah Anda yakin ingin menghapus data pegawai ini?")) {
		try {
			alert("Fungsi Delete Frontend berjalan! (Menunggu Backend)");
		} catch (err) {}
	}
};
...
const selectLimit = (newLimit) => {
	...
	currentPage.value = 1; // Reset ke hal 1 saat ganti limit
	isLimitOpen.value = false;
	fetchUsers();
};

// Pastikan klik di luar menutup dropdown
onMounted(() => {
...
.fade-leave-to {
	...
	transform: translateY(
		10px
	); /* Muncul dari bawah karena letaknya di atas button */
}

/* Scrollbar table agar tetap rapi */
