<template>
    <div class="p-4 md:p-8 h-full flex flex-col relative animate-fade-in">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h2 class="text-2xl font-black text-slate-900 tracking-tight">Manajemen Peminjaman</h2>
                <p class="text-slate-500 mt-1 text-sm font-medium">Kelola persetujuan, antrian harian, dan riwayat alat lab.</p>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-4 items-end">
                <div class="w-full lg:w-auto flex-1">
                    ...
                    <input v-model="startDate" type="date"
                        class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 bg-slate-50 outline-none focus:ring-4 focus:ring-blue-50 transition-all cursor-pointer" />
                </div>
                <div class="w-full lg:w-auto flex-1">
                ...
                <div class="flex gap-2 w-full lg:w-auto flex-wrap sm:flex-nowrap">
                    <button @click="setToday"
                        class="flex-1 lg:flex-none px-4 py-2.5 bg-emerald-50 text-emerald-700 text-xs font-black rounded-xl border border-emerald-100 hover:bg-emerald-100 transition-all cursor-pointer">Hari Ini</button>
                        ...
                        <tr v-else v-for="item in paginatedData" :key="item.id" class="border-b border-slate-50 hover:bg-slate-50/50">
                            ...
                            <td class="p-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <ExclamationTriangleIcon v-if="item.laporan_masalah?.length > 0" class="w-4 h-4 text-orange-500" title="Ada Laporan Masalah!" />
                                </div>
                                <div class="flex items-center gap-2 mt-1">
                                    ...
                                    <span v-if="item.kategori_kebutuhan === 'Khusus'" class="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 px-1.5 py-0.5 rounded">Khusus</span>
                                </div>
                            </td>
                            <td class="p-4">
            ...
            <div v-if="!isLoading && filteredData.length > 0"
                ...
                <div class="flex items-center gap-2">
                    <button @click="currentPage--" :disabled="currentPage === 1"
                        class="px-3 py-1.5 text-sm font-bold rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition cursor-pointer">
                        Sebelumnya
                    </button>
...
import ModalManagementPeminjam from '../components/ModalManagementPeminjam.vue'; // IMPORT MODAL
...
// FUNGSI INI DIPANGGIL SAAT TOMBOL DI DALAM MODAL DI-KLIK (EVENT EMIT)
const updateStatus = (newStatus) => {
    let pesanKonfirmasi = `Apakah Anda yakin ingin mengubah status menjadi "${newStatus}"?`;

    if (newStatus === 'Disetujui') {
        pesanKonfirmasi = 'Setujui permohonan ini? Nomor surat akan di-generate otomatis oleh sistem.';
    } else if (newStatus === 'Dipinjam') {
        pesanKonfirmasi = 'Pastikan KTM atau jaminan telah diterima. Ubah ke status Sedang Dipinjam?';
    ...
    showAlert(
        ...
        async () => {
            ...
            processingStatus.value = newStatus; // Mulai Loading

            try {
                const response = await api.put(`/admin/peminjaman/${selectedData.value.id}/status`, { status: newStatus });
                showAlert(`Status berhasil diubah menjadi ${newStatus}`, 'success');
                isModalOpen.value = false;
                await fetchPeminjaman(); // Refresh tabel dari backend
            } catch (error) {
                console.error("Error Update Status:", error);
                const errMsg = error.response?.data?.message || 'Gagal mengubah status di server';
                showAlert(errMsg, 'error');
            } finally {
                processingStatus.value = null; // Matikan Loading
            }
        }
    );
};
